...
SEND_IP="192.168.1.240"
SEND_PORT="15999"
RECEIVE_PORT="15999"

# GCS computer (May be the same as the Matlab computer)
...
sed "s/TEMPLATE_SEND_PORT/${SEND_PORT}/g" aux > mavlink-router.conf
cp mavlink-router.conf ./aux
sed "s/TEMPLATE_SEND_IP/${SEND_IP}/g" aux > mavlink-router.conf
cp mavlink-router.conf ./aux
sed "s/TEMPLATE_RECEIVE_PORT/${RECEIVE_PORT}/g" aux > mavlink-router.conf
cp mavlink-router.conf ./aux
sed "s/TEMPLATE_GCS_IP_ADDRESS/${GCS_IP}/g" aux > mavlink-router.conf
rm aux
...
if tmux list-sessions -F "#{session_name}" | grep -qw "work_${HOST}"; then
    ...
    tmux send-keys -t work_${HOST} "sleep 2" C-m
    VRPN_TOPIC="/vrpn_client_ros/${SUBJECT}/pose"
    MAVROS_TOPIC="/${SUBJECT}/mavros/vision_pose/pose"
    COMMAND="rosrun topic_tools drop ${VRPN_TOPIC} 1 2 ${MAVROS_TOPIC}"
    tmux send-keys -t work_${HOST} "${COMMAND}" C-m
    ...
    tmux send-keys -t work_${HOST} "sleep 2" C-m
    OPTIONS="_fcu_url:=tcp://localhost:5760 _system_id:=${MAVROS_SYS_ID}"
    COMMAND="ROS_NAMESPACE=${SUBJECT} rosrun mavros mavros_node ${OPTIONS}"
    tmux send-keys -t work_${HOST} "${COMMAND}" C-m
    ...
    tmux send-keys -t work_${HOST} "${START_ROUTERD_CMD} & disown" C-m
